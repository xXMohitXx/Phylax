...
def cmd_bless(args):
    ...
    from phylax.server.storage.files import FileStorage
    from phylax._internal.errors import NonDeterministicGoldenError
    ...
    # Phase 1: Fail if trace has no verdict (meaningless golden)
    if trace.verdict is None:
        print(f"[PHYLAX_E201] Cannot bless trace without verdict.")
        print(f"   Trace '{args.trace_id}' has no expectations evaluated.")
        print(f"   A golden trace must have a deterministic verdict.")
        return 1
    
    # Phase 1: Fail if trace verdict failed (unstable golden)
    if trace.verdict.status == "fail":
        print(f"[PHYLAX_E201] Cannot bless a failing trace.")
        print(f"   Trace '{args.trace_id}' has verdict: FAIL")
        print(f"   Violations: {trace.verdict.violations}")
        print(f"   A golden trace must pass all expectations.")
        return 1
