...
jobs:
  check:
    ...
    steps:
      ...
      <CodeBlock language="python" title="enforcement_modes.py" code={`from phylax import ModeHandler, EnforcementMode, VALID_MODES
...
# result.exit_code = 0`} />
      ...
      <CodeBlock language="python" title="ci_artifacts.py" code={`from phylax import (
...
code = resolve_exit_code(verdict="FAIL", mode="observe")  # → 0`} />
      ...
      <CodeBlock language="python" title="meta_enforcement.py" code={`from phylax import (
...
# result.passed = False, result.removed = {"e3"}`} />
      ...
      <h2 className="text-2xl font-semibold text-coffee-bean mt-8 mb-4">Error Codes</h2>
      <div className="overflow-x-auto">
        <table className="w-full text-sm border-collapse">
          <thead><tr className="border-b border-black/10">
            <th className="text-left py-3 pr-4 font-semibold text-coffee-bean">Code</th>
            <th className="text-left py-3 pr-4 font-semibold text-coffee-bean">Description</th>
            <th className="text-left py-3 font-semibold text-coffee-bean">Action</th>
          </tr></thead>
          <tbody className="text-coffee-bean/80">
            <tr className="border-b border-black/5"><td className="py-3 pr-4"><code className="text-xs">PHYLAX_E101</code></td><td className="py-3 pr-4">Function has no expectations</td><td className="py-3">Add @expect decorator</td></tr>
            <tr className="border-b border-black/5"><td className="py-3 pr-4"><code className="text-xs">PHYLAX_E102</code></td><td className="py-3 pr-4">Execution graph is empty</td><td className="py-3">Ensure graph has nodes</td></tr>
            <tr className="border-b border-black/5"><td className="py-3 pr-4"><code className="text-xs">PHYLAX_E201</code></td><td className="py-3 pr-4">Cannot bless failed trace</td><td className="py-3">Fix expectations first</td></tr>
            <tr className="border-b border-black/5"><td className="py-3 pr-4"><code className="text-xs">PHYLAX_E202</code></td><td className="py-3 pr-4">No golden traces</td><td className="py-3">Bless a trace first</td></tr>
            <tr><td className="py-3 pr-4"><code className="text-xs">PHYLAX_E203</code></td><td className="py-3 pr-4">Hash mismatch with golden</td><td className="py-3">Investigate regression</td></tr>
          </tbody>
        </table>
      </div>
    </div>
