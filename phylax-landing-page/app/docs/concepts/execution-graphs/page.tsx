...
export default function ExecutionGraphsPage() {
  return (
    <div className="flex flex-col gap-6 w-full">
      ...
      <CodeBlock language="python" title="execution_graph.py" code={`from phylax import trace, expect, execution
...
#   [classify] → [research] → [respond]`} />
      ...
      <CodeBlock language="python" title="Graph Verdict Model" code={`class GraphVerdict:
    status: "pass" | "fail"
    ...
    tainted_count: int              # Nodes downstream of failures`} />
      ...
      <CodeBlock language="python" title="graph_analysis.py" code={`from phylax import ExecutionGraph
...
diff = GraphDiff.compute(graph_a, graph_b)`} />
      ...
      <CodeBlock language="python" title="Node Configuration" code={`from phylax import NodeRole, GraphStage
...
GraphStage.OUTPUT       # Response generation`} />
      ...
      <CodeBlock language="bash" title="Terminal" code={`# Start the server
phylax server

# Open http://127.0.0.1:8000/ui
# Navigate to an execution to see its graph`} />
      <CodeBlock language="bash" title="API Endpoints" code={`GET /v1/executions                      # List executions
