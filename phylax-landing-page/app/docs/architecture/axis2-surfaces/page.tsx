...
export default function Axis2Page() {
  return (
    <div className="flex flex-col gap-6 w-full">
      ...
      <CodeBlock language="bash" title="Directory Structure" code={`phylax/_internal/surfaces/
...
└── stability.py               # Phase 2.4: Cross-run stability`} />
      ...
      <CodeBlock language="python" title="Structured Rules" code={`FieldExistsRule("data.user.email")          # Field must exist
...
ArrayBoundsRule("data.items", min_length=1, max_length=50)`} />
      ...
      <CodeBlock language="python" title="Tool Rules" code={`ToolPresenceRule("fetch_database")           # Tool must be called
...
ToolOrderingRule(["search", "fetch", "format"])  # Call order`} />
      ...
      <CodeBlock language="python" title="Execution Rules" code={`StepCountRule(min_steps=3, max_steps=10)     # Step count bounds
...
RequiredStageRule(["validation", "safety"])    # Required stages`} />
      ...
      <CodeBlock language="python" title="Stability Rules" code={`ExactStabilityRule()           # Output must be identical across runs
